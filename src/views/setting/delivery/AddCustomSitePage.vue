<template>
    <div class="p-container">
            ...
            <a-row type="flex" justify="center">
                <a-form-model ref="entity" style="width: 100%" :model="form" :rules="formRules" :labelCol="{span:3}"
                    ...
                    <a-form-model-item label="营业时间" required>
                        ...
                        <p-select-time v-model="form.openOption" :is-request-completed="true"
                                       @exception="onOpenCatch"
                                       v-if="form.openMode === deliveryTimeModeOptions.custom"/>
                    </a-form-model-item>
                    ...
                    <a-form-model-item v-if="form.isSelectTime" label="最长预约" required>
                        <a-radio-group v-model="form.maxReservationMode" class="radio-list">
                            ...
                            <a-radio :value="true">
                                <span>可预约</span>
                                <a-select :disabled="!form.maxReservationMode" style="width: 80px"
        ...
        <p-footer>
            <template #content>
                <a-button :loading="loading" @click="onSave" type="primary" class="font-size-12 p-input-right">保存
                </a-button>
    ...
    methods: {
        onSave() {
            this.$refs.entity.validate((valid) => {
                if (valid) {
                    ...
                    console.log(entity)
                    selfTakeDeliveryService.add(entity).then(res => {
                        this.loading = false
        ...
        toEntity() {
            ...
            return {
                ...
                districtCode: this.form.address[2],
                lat: this.form.lat,
                lng: this.form.lng,
