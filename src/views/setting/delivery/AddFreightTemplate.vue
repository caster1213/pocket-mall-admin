        ...
        <a-row type="flex" justify="center">
            <a-form style="width: 100%" :form="form" :labelCol="{span:3}" :wrapperCol="{span:21}">
                ...
                <a-form-item :required="true" label="模板类型">
                    <a-radio-group @change="onChangeFreightType" v-decorator="['freightType',{initialValue:'1'}]">
                ...
                <a-form-item :required="true" label="计价方式">
                    <a-radio-group @change="onChangeChargeType" v-decorator="['freightChargeType',{initialValue:'1'}]">
    ...
    methods: {
        onSubmit() {
            ...
            this.form.validateFields((err, value) => {
                ...
                freightTemplateService.add(value).then(res => {
        ...
        onAreaSelectCompleted() {
            ...
            let counter = (nodes, depth) => {
                if (nodes.length === 0) return
                depth += 1
                for (let node of nodes) {
                    if (depth === 3 && node.checked) {
                        this.currentRow.selectNodes.push(node.key)
                    }
                    counter(node.children, depth)
                }
            }
            counter(this.currentRow.treeNodes, 0)
        ...
        appendRow() {
            ...
            if (this.freightType === constant.freightType.sell) {
                ...
                row.firstUnit = new Field('firstUnit', '80%', 'number', 'input', '', [required, fun])
                row.firstUnitPrice = new Field('firstUnitPrice', '80%', 'number', 'input', '', [required, decimal])
                row.nextUnit = new Field('nextUnit', '80%', 'number', 'input', '', [required, fun])
                row.nextUnitPrice = new Field('nextUnitPrice', '80%', 'number', 'input', '', [required, decimal])
            }
            this.rows.push(row)
        },
        initTreeNode() {
