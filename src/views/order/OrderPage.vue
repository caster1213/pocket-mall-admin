    ...
    <div class="p-container">
        <div style="margin-bottom: 60px;" class="p-box p-box-bg-gary">
            <div class="p-box p-box-bg-white">
                <a-row type="flex" justify="space-between">
                    <a-row type="flex">
                        <a-space>
                            ...
                            <a-switch v-model="filter.isFilter"
                                      class="font-size-12 p-input-right"></a-switch>
                            ...
                            <a-button :disabled="useBulkBtn" class="font-size-12  p-input-left">批量标记</a-button>
                            <a-button @click="onExportOrder" class="font-size-12  p-input-left">订单导出</a-button>
                        </a-space>
                    </a-row>
                    <a-row type="flex">
                        <a-input-group style="display: inline-block;" compact>
                            <a-select v-model="filter.searchOption" default-value="1" style="width: 100px"
                                ...
                                <a-select-option value="userName">客户名称</a-select-option>
                                ...
                                <a-select-option value="expressCode">快递单号</a-select-option>
                            </a-select>
                ...
                <a-row v-show="filter.isFilter" style="flex-direction: column" type="flex">
                    <div class="order-filter-row">
                        <label class="font-size-12 p-input-right">订单来源</label>
                        <a-radio-group v-model="filter.orderSource" defaultValue="" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                            <a-radio-button value="1">公众号</a-radio-button>
                            <a-radio-button value="2">小程序</a-radio-button>
                            <a-radio-button value="3">App</a-radio-button>
                            <a-radio-button value="4">H5</a-radio-button>
                        </a-radio-group>
                    </div>
                    <div class="order-filter-row">
                        <label class="font-size-12 p-input-right">营销类型</label>
                        <a-radio-group v-model="filter.orderGenre" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                            <a-radio-button value="1">常规</a-radio-button>
                            <a-radio-button value="4">拼团</a-radio-button>
                            <a-radio-button value="3">分销</a-radio-button>
                    ...
                    <div class="order-filter-row">
                        ...
                        <a-radio-group v-model="filter.productGenre" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                            <a-radio-button value="1">实物商品</a-radio-button>
                            <a-radio-button value="2">虚拟商品</a-radio-button>
                        </a-radio-group>
                    </div>
                    <div class="order-filter-row">
                        <label class="font-size-12 p-input-right">订单状态</label>
                        <a-radio-group v-model="filter.orderState" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                    ...
                    <div class="order-filter-row">
                        ...
                        <a-radio-group v-model="filter.rebateState" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                            <a-radio-button value="2">售后中</a-radio-button>
                            <a-radio-button value="3">售后结束</a-radio-button>
                        </a-radio-group>
                    </div>
                    <div class="order-filter-row">
                        <label class="font-size-12 p-input-right">支付方式</label>
                        <a-radio-group v-model="filter.payGenre" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                            <a-radio-button value="1">微信支付</a-radio-button>
                            <a-radio-button value="4">支付宝</a-radio-button>
                            <a-radio-button value="2">余额支付</a-radio-button>
                        </a-radio-group>
                    </div>
                    <div class="order-filter-row">
                        <label class="font-size-12 p-input-right">配送方式</label>
                        <a-radio-group v-model="filter.deliveryGenre" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                            <a-radio-button value="1">商家配送</a-radio-button>
                            <a-radio-button value="2">到店自提</a-radio-button>
                        </a-radio-group>
                    </div>
                    <div class="order-filter-row">
                        <label class="font-size-12 p-input-right">标记筛选</label>
                        <a-radio-group v-model="filter.marketGenre" class="font-size-12">
                            <a-radio-button value="">不限</a-radio-button>
                            <a-radio-button style="padding: 0 8px" value="1">
                                <a-icon :style="{fontSize:'16px'}" :component="FlagIcon"></a-icon>
                            </a-radio-button>
                            <a-radio-button style="padding: 0 8px" value="2">
                                <a-icon :style="{fontSize:'18px'}" :component="FlagYellow"></a-icon>
                            </a-radio-button>
                            <a-radio-button style="padding: 0 8px" value="3">
                                <a-icon :style="{fontSize:'18px'}" :component="FlagGreen"></a-icon>
                            </a-radio-button>
                            <a-radio-button style="padding: 0 8px" value="4">
                                <a-icon :style="{fontSize:'18px'}" :component="FlagBlue"></a-icon>
                            </a-radio-button>
                            <a-radio-button style="padding: 0 8px" value="5">
                                <a-icon :style="{fontSize:'18px'}" :component="FlagViolet"></a-icon>
                            </a-radio-button>
                        </a-radio-group>
                    </div>
                    <div class="order-filter-row">
                        <a-button type="primary" class="font-size-12 p-input-right" @click="getDataSource">搜索</a-button>
                        <a-button class="font-size-12" @click="onClearFilter">重置</a-button>
                    </div>
                </a-row>
            </div>
                ...
                <a-spin :spinning="filter.loading">
                    <table class="order-table">
                        ...
                        <template v-if="dataSource.length >0" v-for="row in dataSource">
                                ...
                                <td colspan="6" style="border-bottom: 2px solid #F2F2F6">
                                    <a-row type="flex" justify="space-between">
                                        <a-space>
                                            ...
                                            <a-icon v-if="row.lock" type="lock" style="font-size: 13px" />
                                        </a-space>
                                        <div class="detail">
                                            <a-space>
                                                <router-link
                            ...
                            <tr :key="row.id+'i'">
                                ...
                                <td style="max-width: 250px;padding-bottom: 10px;padding-left: 0">
                                    <a-row v-for="(item,index) in row.orderItem" :key="index"
                                        ...
                                        <a-col style="margin-left: 10px;width: calc(100% - 100px)">
                                            <a-row type="flex" style="flex-direction: column">
                                                ...
                                                <span v-if="item.sku.length>0"
                                                      class="font-size-12">规格：{{item.sku}}</span>
                                                <span style="padding-top: 5px" class="font-size-12">单价：¥{{item.productPrice.toDecimalString()}}</span>
                                            </a-row>
                                        </a-col>
                                    </a-row>
                                    <a-button v-show="row.orderItem.length>2" type="link" @click="()=>{
                                       if(row.isCollapse){
                                         row.collapseCount = 2
                                ...
                                <td>
                                    ...
                                    <div v-if="row.payGenre === 3" class="font-size-12">支付宝</div>
                                    <div v-if="row.payGenre === 4" class="font-size-12">云闪付</div>
                                    <div v-if="row.payGenre === 5" class="font-size-12">京东支付</div>
                                </td>
                                <td>
                                    <p-order-status-box @delivered="onDelivered(row.orderNumber)"
                        ...
                        <tr v-if="dataSource.length ===0">
                            <td align="center" colspan="6">暂无订单数据</td>
                        </tr>
                        </tbody>
                    </table>
                </a-spin>
